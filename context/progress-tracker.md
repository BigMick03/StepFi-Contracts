...
### Liquidity Pool Defense-in-Depth Caps (fund_loan hardening)
- **Problem:** `fund_loan()` honored `transfer(pool → merchant, amount)` for whatever merchant the registered creditline supplied, with zero independent verification. The pool's solvency rested entirely on the creditline contract being bug-free; a compromised creditline admin could repoint `set_creditline()` and drain the pool instantly, with `absorb_loss()` erasing the accounting trace.
- **Per-ledger outflow cap:** cumulative `fund_loan` outflows within a single ledger are bounded to `max_outflow_bps` (basis points) of the available-liquidity snapshot taken when the rolling window begins. The window resets whenever the ledger sequence moves. Default `10000` bps = disabled (backward compatible); enforce via `set_max_outflow_bps` (admin-only, rejects > 10000). New error `LedgerOutflowCapExceeded = 14`.
- **Single-recipient concentration cap:** cumulative funded-per-merchant totals are tracked in persistent storage (with TTL extension) and capped at `max_per_merchant` (admin-only setter, rejects negatives; `0` = disabled). New error `MerchantConcentrationCapExceeded = 15`.
- **Optional vendor cross-check:** `set_vendor_registry` (admin-only, accepts `Option<Address>`) registers a vendor-registry contract; when set, `fund_loan` requires `is_active(merchant)` via `try_invoke_contract` before transferring (fails closed — unregistered/unresponsive registry rejects funding). Unset = legacy behavior. New errors `VendorNotActive = 12`, `VendorNotRegistered = 13`. Wiring is via the admin setter only — `initialize()` was deliberately left unchanged (4-arg signature) to avoid breaking the deploy script / deployed contract; the setter satisfies the "initialize()/admin setter" requirement.
- **Granular event:** `emit_loan_funded` now publishes topics `(LQFUND, creditline, merchant)` and data `(amount, remaining_ledger_cap, remaining_merchant_cap)` — headroom after each funding for indexer monitoring (`0` when a cap is disabled).
- **Semantics note:** caps default to disabled (backward compatible — "honest flows unchanged"). Recommended production hardening: call `set_max_outflow_bps(2500)` (25%/ledger) and `set_max_per_merchant(<ceiling>)` after deployment. A malicious creditline is then bounded to a fraction of available liquidity per ledger instead of draining the pool instantly.
- **Tests (18 new, 113 total in liquidity-pool-contract, all green):** repeated-call outflow cap with rejection not consuming headroom, rolling-window reset on new ledger (`set_sequence_number`), default-disabled regression, setter validation (bps > 10000, negative per-merchant) and non-admin guards, per-merchant cap incl. per-merchant isolation and rejected-attempt state, vendor check approved/unregistered/suspended/unset/clear-roundtrip and non-admin guard, and LQFUND event payload (merchant topic + remaining caps).
- **Verification:** `cargo test -p liquidity-pool-contract` → 113 passed, 0 failed; `cargo test --workspace` → all 6 crates green (356 tests, 0 failed); `cargo clippy -p liquidity-pool-contract -- -D warnings` clean; full `wasm32-unknown-unknown --release` workspace build succeeds. (Pre-existing clippy warnings in `vouching-contract`/`creditline-contract`/`vendor-registry-contract`/`parameters-contract` dead-code and `manual-saturating-arithmetic` lints predate this change — untouched.)
- **Files:** `contracts/liquidity-pool-contract/src/lib.rs`, `storage.rs`, `types.rs`, `errors.rs`, `events.rs`, `tests.rs`, `Cargo.toml` (dev-dep on vendor-registry-contract for tests)
